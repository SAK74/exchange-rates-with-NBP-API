...
interface RateC { effectiveDate: string; ask: number; bid: number; }
interface RateAB { effectiveDate: string; mid: number; }
...
export default function Graff(props: GraffProps) {
  ...
  const WIDTH = 500; const HEIGHT = 370; const gradient = 0.9;
  const arr: number[] = [];
  let MAX = 0, MIN = Infinity, maxDay = '', minDay = '';
  ...
  if (props.table === 'C') {
    ...
    for (let i = 0; i < state.length; i++) {
      ...
      if (middle > MAX) {
        const maxItemLength = Math.max(item.ask.toString().length, item.bid.toString().length);
    ...
    MIN = Math.min(...arr);
    let i = 0;
    while ((state[i] as RateAB).mid !== MAX) { i++; }
    maxDay = state[i].effectiveDate;
    i = 0;
    while ((state[i] as RateAB).mid !== MIN) { i++; }
